...
### Review Findings (2026-09-20, closeout review)

- [x] [Review][Patch] `non_empty` in `directory.rs`'s `RawStation`→`DirectoryStation` conversion checked for blank/whitespace-only strings but stored the untrimmed value, so a field like `" Germany"` would render with leading whitespace [winradio/src-tauri/src/directory.rs:82] — fixed: now stores the trimmed value
- [x] [Review][Patch] `parse_stations` had no id-based dedup (unlike `parse_names`'s own seen-set pass), so a directory response repeating a `stationuuid` would reach the frontend as a duplicate Vue `:key` [winradio/src-tauri/src/directory.rs:128] — fixed: added an order-preserving dedup pass mirroring `parse_names`
- [x] [Review][Patch] Zero-match message always read `No stations found for ''.` for a filter-only search (empty text query, e.g. genre-only) [winradio/src/App.vue:132-134] — fixed: falls back to "No stations match your filters." when `lastQuery` is empty
- [x] [Review][Defer] `showFilterControls` never resets when switching away from/back to Search via its own nav icon (only the Filter icon toggles it) [winradio/src/App.vue] — deferred, product call not an unambiguous fix
- [x] [Review][Defer] Old results stay visible with no loading indicator while a re-search is in flight (`results.length > 0` branch precedes the `loading` branch) [winradio/src/App.vue] — deferred, UX polish
- [x] [Review][Defer] No minimum query length or distinct 429 handling in `search_stations` [winradio/src-tauri/src/directory.rs] — deferred, low real-world impact
- [x] [Review][Defer] `search_stations` reachable with zero criteria via direct IPC (not through any current UI call site) [winradio/src-tauri/src/directory.rs:192-219] — deferred, defense-in-depth only
- [x] [Review][Dismiss] Several findings duplicated the four already-recorded `deferred-work.md` entries from the original review round (frontend test coverage of debounce/dedup/`requestSeq`, in-flight request cancellation, filter-dropdown loading/error state) — not re-recorded
- [x] [Review][Dismiss] "Filter selects bypass debounce" — intentional, documented in `search.ts`'s own comment ("Discrete select changes apply immediately too; only free-typed text needs debouncing")
- [x] [Review][Dismiss] Offline-message trailing-period mismatch vs. I/O matrix — cosmetic, matches an inconsistency in the spec's own text
